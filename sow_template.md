...
**Target Server Domain:** `server.[james].edu`  
...
- **Game Summary:** The game starts off with a random player either "X's" or "O's". The player who goes first will choose a column and row to first put their X or their O. Then the second player would go, they aren't able to put their character on the other players character, but have to once more name column and row to place their symbol. These symbols will stack in each row and column, and whoever stacks or lines up 4 of the same symbol wins!
### 1.2 Core Game Rules & Win/Draw Conditions
- **Turn Mechanics:** Choosing whoever starts will be random, either a coin flip or a random choice system, then whoever is selected they will be automatically assigned an X or an O, whoever the random chooser selects, will be the player that goes first, then after that player goes the next goes, and so on.
- **Victory Condition:** The first player to stack or line up 4 of their symbol's in either a horizontal, veritcal or diagonal line will win.
- **Draw/Tie Condition:** In the event of a draw or a tie, the game will reset and choose the opposite player of who was chosen last game. To detect whether or not there is this conditon, I will save the board after every turn, and run conditionals that have to be met in order to continue or end the game (in the event of a tie). The board will be scanned and then ran through an iteration to see if certain tie or draw conditions are met.
